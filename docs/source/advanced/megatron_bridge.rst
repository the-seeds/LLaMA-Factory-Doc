...
`Megatron Bridge <https://docs.nvidia.com/nemo/megatron-bridge/latest/>`__ 是 NVIDIA 提供的 Hugging Face ↔ Megatron-Core 桥梁。LlamaFactory 用它走独立的 PT / SFT 训练路径，而不是 Hugging Face Trainer。

启用方式：设置环境变量 ``USE_MEGATRON_BRIDGE=1``。启动器会强制 ``FORCE_TORCHRUN=1``。

.. warning::
   这与 ``USE_MCA=1`` 是两个后端，不要同时开启。MCA 详见 `mcore_adapter <https://github.com/alibaba/ROLL/tree/main/mcore_adapter>`__。
...
推荐先装 PyTorch 与 `TransformerEngine <https://github.com/NVIDIA/TransformerEngine>`__，再安装 Megatron Bridge：
...
也可以使用 `NeMo Framework 容器 <https://catalog.ngc.nvidia.com/orgs/nvidia/containers/nemo/tags>`__，或构建本仓库的 CUDA 镜像：
...
APEX 的 ``fused_weight_gradient_mlp_cuda`` 是可选依赖。未安装时，LlamaFactory 会自动关闭 ``gradient_accumulation_fusion``，训练仍可继续。
...
.. list-table::
   ...
   * - ``account_for_loss_in_pipeline_split``
     - PP 切分是否计入 loss
   * - ``bias_activation_fusion`` / ``apply_rope_fusion`` / ``masked_softmax_fusion`` / ``cross_entropy_loss_fusion``
     - 设为 ``None`` 时保留 Megatron provider 默认值
...
.. list-table::
   ...
   * - 字段
     - 默认值
     - 说明
   * - ``use_packed_sequences``
     - ``false``
     - SFT packed sequence
   * - ``megatron_pretrained_checkpoint``
     - ``None``
     - 已有 Megatron 格式权重；未设置时自动从 HF 转换
   * - ``export_hf_on_finish``
     - ``false``
     - 训练结束后导出 Hugging Face 权重
   * - ``extra_config``
     - ``None``
     - JSON 字符串或 JSON 文件，支持点路径覆盖
...
LlamaFactory 仍负责数据集加载与 ``template``。训练开始前，rank 0 会把对齐后的样本导出到 ``output_dir/mb_dataset/``：
...
.. list-table::
   ...
   * - ``pt``
     ...
     - ``{"text": "..."}``
   * - ``sft``
     - ``training.jsonl``、可选 ``validation.jsonl``
     - 优先 Hugging Face ``messages``；无法注入 ``{% generation %}`` 时回退 ShareGPT
...
- 部分 GPU（例如 V100）保存 distributed optimizer 时，异步 D2H copy 可能失败；LlamaFactory 会改用同步拷贝。
- pip 安装的 ``megatron-core`` 若已带 ``helpers_cpp``，会跳过源码树里的 ``make`` 编译。
- Web UI 可以提交同一份 YAML，但仍需在启动环境中设置 ``USE_MEGATRON_BRIDGE=1``。

更多上游概念见 `Megatron Bridge 文档 <https://docs.nvidia.com/nemo/megatron-bridge/latest/>`__。
