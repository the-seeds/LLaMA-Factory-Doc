...
.. list-table:: DataArguments
   ...
   * - dataset_dir
     ...
     - 存储数据集的文件夹路径，可以是字符串或字典。  
       ...
       **当为字符串时：**  
       表示数据集目录的路径，例如：  
       `data <https://github.com/hiyouga/LLaMA-Factory/tree/main/data>`_ 。

       **当为字典时：**  
       将覆盖默认从本地 `dataset_info.json <https://github.com/hiyouga/LLaMA-Factory/blob/main/data/dataset_info.json>`_ 加载的行为。其格式如下：
