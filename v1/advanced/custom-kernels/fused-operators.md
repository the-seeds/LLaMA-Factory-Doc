...
在 LLaMA-Factory 中，通过 `NpuMoEFusedMoEKernel` 提供使能该融合算子的入口。只需要调用 `apply_kernel(model, NpuMoEFusedMoEKernel)` 即可针对已适配的模型使能 NPU MoE 融合算子。对于未适配的模型，您也可以参考上述示例代码以及[开发者文档](../../dev-guide/plugins/model-plugins/kernels.md)自行适配。
